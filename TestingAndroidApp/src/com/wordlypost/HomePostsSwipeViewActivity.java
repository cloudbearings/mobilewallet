...
public class HomePostsSwipeViewActivity extends ActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			ArrayList<PostRowItem> rowItems = (ArrayList<PostRowItem>) getIntent().getSerializableExtra("postDetails");
			if (rowItems.size() > 0) {
				generateOffers(rowItems);
			}
	...
	private void getPostsFromServer() {
		if (getIntent().getIntExtra("categoryId", 0) != 0
			...
			BuildService.build.getCategoriesPosts(categoryId, sulg, page, new Callback<String>() {
						...
						if (obj.getJSONArray("posts").length() > 0) {
							...
							for (int i = 5; i < categotyPosts.length(); i++) {
								...
								homeDAO.insertPosts(categotyPost.getInt("id"), categotyPost
										...
										categotyPost.getString("excerpt"), categoryId, sulg, "",
